...
module.exports = {
  ...
  async execute(interaction, client) {
    // Check user roles
    if (!interaction.member.roles.cache.some(role => ['Sr. Moderator', 'Admin', 'Owner'].includes(role.name))) {
    ...
    const upload = interaction.options.getAttachment("file");
        ...
        function getMonthlyData(jsonData) {
          ...
          jsonData.monthly_playtime.forEach(item => {
            ...
            console.log('Monthly Playtime:', item.username, item.value);
          });
          jsonData.monthly_votes.forEach(item => {
            ...
            console.log('Monthly Votes:', item.username, item.value);
          });
          return result;
        }
        ...
        const formattedClosestMonthEnd = `${monthNames[closestMonthEnd.getMonth()]} ${closestMonthEnd.getFullYear()}`;
    
        // Create the embeds
        const PlaytimeEmbed = new EmbedBuilder()
          .setColor(0x0099FF)
          .setDescription(`## Monthly Playtime Leaderboard results - ${formattedClosestMonthEnd}`);
    
        const VotesEmbed = new EmbedBuilder()
          .setColor(0x0099FF)
          .setDescription(`## Monthly Votes Leaderboard results - ${formattedClosestMonthEnd}`);
        ...
        PlaytimeEmbed.addFields(
          ...monthlyData.monthly_playtime.map((item, index) => {
            ...
            const minutes = Math.round((item.value * 0.05 / 60 / 60 - hours) * 60);
            return { name: `${numberEmojis[index]} ${item.username.replace(/_/g, '\\_')}`, value: `\`\`\`${hours} Hrs & ${minutes} Mins\`\`\``, inline: true };
          })
        );
    
        // Add fields for votes data
        VotesEmbed.addFields(
          ...monthlyData.monthly_votes.map((item, index) => ({ name: `${numberEmojis[index]} ${item.username.replace(/_/g, '\\_')}`, value: `\`\`\`${item.value.toString()} Votes\`\`\``, inline: true }))
        );
